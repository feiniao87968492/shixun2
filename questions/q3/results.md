...
q3 复用 q2 固定划分，train=514、test=221，未重新划分数据。依赖审计见 `questions/q3/artifacts/tables/q3_dependency_audit.csv`，q2 carry/apex 模型、q2 ODE 参数、q2 元数据和 q2 validation 均通过检查；q3 ODE 复核条件为 `q3_ode_verified=true`。task7 后依赖审计扩展为 16 项，并记录当前 q2 run metadata SHA256、q2 ODE 参数 SHA256 和 `forward_x` carry 定义。
...
task7 生成 `docs/reproducibility/q2_q3_release_manifest.json`，记录当前 Git commit、`configs/default.yaml`、`data/processed/golf_shots_clean.csv`、Q2/Q3 `run_metadata.json` 和所有 Q2/Q3 核心 CSV 的 SHA256。新增集成验证 `tests/test_q2_q3_integration.py` 检查 Q3 使用的 Q2 metadata hash、carry 定义、ODE 复核候选积分成功和 README 中的推荐参数可由 CSV 复算。
