...
参数用训练集代表样本粗网格标定，并以 `carry_definition=forward_x` 的目标函数接有界 Powell 局部优化。标定阶段采用 `max_step=0.15` 加速；最终测试集评估使用配置中的正式求解器参数。

task4 后的有效参数为：drag `C_D=0.05`（边界解）、constant_lift `C_D=0.238654,C_L=0.203952`、spin_factor_lift `C_D=0.050059,k_L=0.151837`。第二问 best-fit ODE 由完整训练集目标自动确定；第三问兼容 ODE 保留为 `spin_factor_lift`，并要求通过边界稳定性检查。
