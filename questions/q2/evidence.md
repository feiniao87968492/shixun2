...
| Q2-C12 | ODE 求解时间上限由配置控制且时间上限耗尽被显式标记 | `configs/default.yaml`、`q2_validation_checks.csv`、`run_metadata.json` | 166 项 q2 validation；`tests/test_q2_q3_integration.py` 覆盖 `time_horizon_exceeded` | supported |

所有 supported 主张均可由 `python questions/q2/scripts/pipeline.py --config configs/default.yaml` 重新生成，并由 `python questions/q2/scripts/validate.py --config configs/default.yaml` 和 `python -m pytest tests/test_q2_q3_integration.py -q` 检查。
