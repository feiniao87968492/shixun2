...
Q2 与 Q3 的正式结果由 `python questions/q2/scripts/pipeline.py --config configs/default.yaml` 和 `python questions/q3/scripts/pipeline.py --config configs/default.yaml` 重新生成。task7 后新增 `docs/reproducibility/q2_q3_release_manifest.json`，记录当前 Git commit、配置、处理数据、Q2/Q3 run metadata 和所有 Q2/Q3 核心 CSV 的 SHA256；`tests/test_q2_q3_integration.py` 验证 Q3 使用的 Q2 metadata hash、`forward_x` carry 定义、ODE 复核积分状态和文档参数均可由 CSV 复算。
